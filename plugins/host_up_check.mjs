...
export default {
  ...
  async run(host) {
    ...
    console.log(`Running Host Up Check on ${host}`);
    const data = [];
    let up = false;
    let os = null;
    ...
    try {
      ...
      console.log(`Executing ping command: ping ${pingArgs.join(' ')}`);
      ...
      if (received) {
        ...
        if (ttlMatch) {
          ...
          console.log(`ICMP TTL ${ttl} candidates: ${icmpCandidates.map(c => c.Device_OS).join(', ')}`);
      ...
      console.error(`ICMP probe error: ${err.message}`);
      data.push({
        probe_protocol: 'icmp',
        probe_port: null,
    ...
    for (const port of tcpPorts) {
      await new Promise((resolve) => {
        console.log(`Attempting TCP probe on ${host}:${port}`);
        ...
        socket.on('connect', () => {
          ...
          setTimeout(() => {
            ...
            if (banner.toLowerCase().includes('unix') || banner.toLowerCase().includes('linux') || banner.toLowerCase().includes('bftpd')) {
              newOs = 'Linux';
              console.log(`TCP probe port ${port}: Detected ${newOs} from banner: ${banner.trim()}`);
              ...
              if (routerMatch) {
                ...
                console.log(`TCP probe port ${port}: Detected router ${newRouterInfo.name} ${newRouterInfo.version} from banner: ${banner.trim()}`);
              } else {
                newRouterInfo = { name: 'Netgear', version: null };
                console.log(`TCP probe port ${port}: Detected generic Netgear router from banner: ${banner.trim()}`);
              }
            } else if (port === 3389) {
              ...
              console.log(`TCP probe port ${port}: Detected ${newOs} from open port (router heuristic)`);
            }
            ...
            if (newOs && (!os || (icmpCandidates.length > 1 && (OS_SPECIFICITY[newOs] || 0) >= (OS_SPECIFICITY[os] || 0)))) {
              console.log(`TCP probe port ${port}: Setting os=${newOs} (ICMP ambiguous or less specific, TTL ${icmpCandidates[0]?.TTL})`);
              os = newOs;
              if (newRouterInfo) {
                router_info = newRouterInfo;
              }
            } else {
              console.log(`TCP probe port ${port}: Keeping os=${os} (newOs=${newOs})`);
            }
            data.push({
              probe_protocol: 'tcp',
              probe_port: port,
    ...
    const udpPort = 54321;
    await new Promise((resolve) => {
      console.log(`Attempting UDP probe on ${host}:${udpPort}`);
    ...
    console.log(`Host Up Check result: up=${up}, os=${os}, router_info=${JSON.stringify(router_info)}, data=${JSON.stringify(data)}`);
