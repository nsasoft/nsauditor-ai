...
test('linkifyBareUrls escapes URL characters via escAttr to prevent XSS (S.3)', async () => {
  // Attack vector: markdown-it renders a URL containing %22 (encoded ") and decodes it to
  // the HTML entity &quot; in the anchor's visible text. stripLiteralAnchors then leaves
  // that &quot; in the text node. linkifyBareUrls re-captures the URL — and before the fix
  // the produced href would be href="...&quot;..." where &quot; is decoded by browsers to "
  // (an attribute boundary break = XSS). escAttr() converts & → &amp;, so &quot; becomes
  // &amp;quot;, which browsers treat as a literal string — not a quote character.
  const urlWithEncodedQuote = 'https://evil.example.com/path?x=%22onmouseover=alert(1)';
  const html = await buildHtmlReport({
    host: '127.0.0.1',
    whenIso: '2025-08-29T12:00:00Z',
    model: 'unit-test',
    md: `Check this link: ${urlWithEncodedQuote}`
  });

  // The URL must appear somewhere in the output
  assert.ok(html.includes('evil.example.com'), 'URL must be present in output');

  // The linkifyBareUrls-generated <a> tag must NOT contain an unescaped &quot; in the href.
  // An unescaped &quot; in an href attribute is decoded by the browser as ", breaking the
  // attribute boundary and enabling XSS. The escaped form &amp;quot; is safe.
  assert.ok(
    !html.includes('href="https://evil.example.com/path?x=&quot;'),
    'href must not contain &quot; (unescaped HTML entity that browsers decode as ")'
  );

  // The safe form (double-encoded) must be present in the linkifyBareUrls-generated anchor
  assert.ok(
    html.includes('href="https://evil.example.com/path?x=&amp;quot;'),
    'href must contain &amp;quot; (safely escaped, browser renders as literal &quot; not ")'
  );
});
