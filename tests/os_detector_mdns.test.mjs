  ...
  const mdnsRows = [
    {
      ...
      response_banner: 'name=Lidiva’s MacBook Air; host=Lidivas-MacBook-Air.local.; deviceid=12:8E:BE:4F:CE:6C; addresses=fe80::27:a274:558e:ba44,192.168.1.15'
    }
  ];
  ...
  const mdnsRows = [
    {
      ...
      probe_port: 5353,
      probe_info: 'airplay._tcp — model=Mac15,13; srcvers=870.14.1',
      response_banner: 'name=Lidiva’s MacBook Air; host=Lidivas-MacBook-Air.local.; addresses=192.168.1.15'
    }
  ];
  const results = [wrap('MDNS Scanner', '016', mdnsRows)];
  const out = await osDetector.run(host, 0, { results, context: { ...ctxHelpers } });
  ...
  const mdnsRows = [
    {
      ...
      response_banner: 'name=EPSON ET-2720 Series; host=EPSONB121D4.local.; addresses=192.168.1.24'
    }
  ];
  const results = [wrap('MDNS Scanner', '016', mdnsRows)];
  const out = await osDetector.run(host, 0, { results, context: { ...ctxHelpers } });
