...
test('HISTORY_FILE is exported and is a non-empty string', () => {
  assert.equal(typeof HISTORY_FILE, 'string');
  assert.ok(HISTORY_FILE.length > 0);
  assert.ok(HISTORY_FILE.endsWith('.jsonl'));
});
...
test('pruneForCE preserves valid JSON entries with null timestamp', async () => {
  const tmpDir = os.tmpdir();
  const filePath = path.join(tmpDir, 'nsa_hist_nullts_' + Date.now() + '.jsonl');

  // A valid JSON line whose timestamp field is null — must survive pruning
  fs.writeFileSync(filePath, JSON.stringify({ host: 'x', timestamp: null }) + '\n');
  try {
    await pruneForCE(filePath);

    const lines = fs.readFileSync(filePath, 'utf8').trim().split('\n').filter(Boolean);
    assert.equal(lines.length, 1, 'Entry with null timestamp should be preserved');
    assert.equal(JSON.parse(lines[0]).host, 'x');
  } finally {
    try { fs.unlinkSync(filePath); } catch {}
  }
});
