...
test('CLI format detection — "csv" is recognized (mirrors cli.mjs wantCsv logic)', () => {
  // Mirror: const wantCsv = outputFormat && String(outputFormat).toLowerCase().includes('csv');
  const detect = (fmt) => fmt && String(fmt).toLowerCase().includes('csv');
  assert.equal(detect('csv'), true);
  assert.equal(detect('CSV'), true);
  assert.equal(detect('sarif'), false);
