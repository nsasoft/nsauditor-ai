...
**Latest: CE 0.1.98 + Enterprise 0.16.7** (May 2026)

- 🌐 **CloudTrail multi-region audit completes + fails closed** (CE 0.1.98 — paired EE 0.16.7) — *(Enterprise fix)* the EE CloudTrail auditor (plugin 1040) enumerates trails across ~32 regions; unreachable regions had no per-region connect timeout, so the pass could hang past Claude Desktop's ~60s tool-call limit and fall back to a single region — silently incomplete. EE 0.16.7 gives each region client a short connect/request timeout + wider fan-out (a dead region fails in ~2s, not ~30s; 234s → ~13s), and an errored region no longer discards every other region's evidence — it's surfaced as an explicit **evidence gap** routed to the CloudTrail controls (the verdict fails closed over it), never a silent miss. Confirmed in production via Claude Desktop. CE is a paired no-op bump. Plugin count UNCHANGED at 28; all six matrices UNCHANGED.
- 🧮 **Compliance-engine over-reporting folds + CloudTrail soft-budget hardening** (CE 0.1.97 — paired EE 0.16.6) — *(Enterprise fix)* EE 0.16.6 fixes a 0.16.5 regression where the CloudTrail soft budget could exceed the CLI per-plugin wall and hard-cancel the auditor (0 findings); the budget is now `0.8 × min(PLUGIN_TIMEOUT_MS, CLOUD_PLUGIN_TIMEOUT_MS)` so it fails closed on both the CLI and MCP paths. Plus two over-report folds: KMS grant-decrypt findings are no longer raised on AWS-managed keys, and positive "substrate evidence" is no longer counted as a control violation. CE is a paired no-op bump. Plugin count UNCHANGED at 28; all six matrices UNCHANGED.
- 🧭 **Compliance-mapping false-clean fixes (8-lens review)** (CE 0.1.96 — paired EE 0.16.5) — *(Enterprise fix)* a full 3-cloud smoke reviewed through 8 adversarial audit-skill lenses fixed four false-cleans: a live `0.0.0.0/0` perimeter now routes cross-framework to PCI / ISO / NIST (was SOC 2 only); an aborted CloudTrail audit fails closed across all CloudTrail-dependent controls; GCP Cloud Storage emits an evidence gap on enumeration failure instead of a silent clean; and internal review markers were scrubbed from the shipped framework JSONs. CE is a paired no-op bump. Plugin count UNCHANGED at 28; all six matrices UNCHANGED.
- 🛡️ **`scan_cloud` now surfaces cloud findings reliably** (CE 0.1.95 — paired EE 0.16.4) — *(false-clean fix)* the tool now builds a `findingsSummary` (per-provider severity counts + the CRITICAL/HIGH list) **directly from the scan results**, instead of the network-host concluder — which is built for port/service scans, doesn't understand cloud compliance findings, and silently **dropped** them. A scan could previously report `audited:true` with **0 findings over real CRITICALs**; now every finding is counted and every CRITICAL/HIGH is listed. Distinct from the 0.1.93 not-audited fold (caught by the `audit-cloud-plugin-false-negatives` review). Plugin count UNCHANGED at 28; all six matrices UNCHANGED.
...
- ☁️ **28 enterprise plugins** across AWS, Azure, GCP — fully integrated SOC 2 / HIPAA / NIST CSF 2.0 / PCI DSS v4.0.1 / ISO 27001 / CIS Controls v8 evidence pipeline (Enterprise).
...
| **CIS Critical Security Controls v8** — per-Safeguard mapping + IG-cumulative discipline (153 Safeguards / 18 Controls) (NEW) | — | — | ✅ |
| Multi-framework `--compliance soc2,hipaa,nist-csf,pci-dss,iso-27001,cis-v8` from one scan | — | — | ✅ |
