...
[![Tests](https://img.shields.io/badge/tests-439%20passing-brightgreen.svg)](#tests)
...
| Pro AI (OpenAI/Claude pipelines) | — | ✅ | ✅ |
...
NSAuditor AI supports two AI providers for vulnerability analysis. AI is optional — the platform is fully functional without it.

**Providers:** OpenAI (GPT-4o) and Anthropic Claude (Sonnet/Opus)

**Prompt modes:** `basic` (simple summary), `pro` (multi-section analysis with CVEs), `optimized` (structured QA checklist with strict evidence validation)
...
For local-only AI (no external API calls), use [Ollama](https://ollama.ai):
...
- **SSRF protection** — private, loopback, and cloud metadata addresses blocked at the CLI boundary
...
Security: SSRF protection on all host inputs, port validation (1–65535), CPE format enforcement, dependency injection for test isolation.
...
| Webhook URL rejected | Private, loopback, and cloud metadata addresses blocked (SSRF protection) |
