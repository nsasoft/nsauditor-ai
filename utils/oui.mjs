...
export function lookupVendor(mac) {
  if (!OUI_DB) {
    // Synchronous fallback to ensure initialization
    console.warn("[oui.mjs] OUI_DB not initialized, attempting to load synchronously");
    initOui().catch(e => console.error("[oui.mjs] Synchronous init failed:", e.message));
  }
