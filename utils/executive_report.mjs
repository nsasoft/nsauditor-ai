...
function renderCoverageSummary(model) {
  ...
  if (unread.length) {
    ...
    // ⚠️ TWO REASONS, TWO SENTENCES — and conflating them put a FALSE QUANTITY in front of a
    // client. A container nobody reads is genuinely absent from this document. A carve-out whose
    // PREMISE BROKE is different: its objects ARE rendered above except for the break, so saying
    // they "are NOT included above" contradicts the body of the same report. Plugin 1150 emits
    // `data: findings`, so one orphan among sixteen mirrors used to announce that seventeen
    // objects were missing from a report containing sixteen of them.
    const broken = unread.some((u) => Object.keys(u.premiseFailures ?? {}).length);
    if (broken) {
      const why = unread.flatMap((u) => Object.values(u.premiseFailures ?? {}));
      lines.push(`<p class="caveat">⚠️ ${total} recorded finding-like object(s) are NOT accounted `
        + `for by this report — ${parts.join(' · ')}. Some containers are not read at all; others `
        + 'are summaries whose link to a rendered finding could not be re-derived on this run, so '
        + 'their objects may appear above under a different heading. '
        + `${escapeHtml(why.join(' · '))}</p>`);
    } else {
...
// Names what `brand` actually was, for the refusal below — never called on a plain object.
// Same voice and job as brand.mjs's own `describeLogoPathType`: a caller told only "must be an
// object" over a value it cannot see has to open this module to learn what it did wrong.
function describeBrand(v) {
...
// ⚠️ REFUSES rather than defaulting, and the choice is the opposite of what "make it total"
// usually means: this renders a CLIENT-FACING deliverable, so silently normalising an absent
// brand into an unbranded report is the WORSE failure — it hands the caller a complete,
// plausible document at exit 0 with the consultant's identity missing, and that is the copy
// that reaches the client.
//
// That is the CC-4 ruling one layer down. `tests/report_command.test.mjs` already pins "a
// value-less --brand is FATAL, not a silent unbranded default", recorded there because a shell
// glob that swallowed the path "produced the IDENTICAL unbranded default report, at exit 0, as
// never asking for a brand at all — silently". An absent argument here is indistinguishable
// from a lost one for exactly the same reason, and refusing costs the honest caller nothing:
// "no branding wanted" stays expressible as `{}`, which renders precisely what it rendered
// before (measured: byte-identical to loadBrand(null)'s defaults, 4579 bytes on the fixture).
//
// WHAT WAS MEASURED, over eight brand shapes, before this guard existed: null / undefined / an
// omitted second argument all threw `TypeError: Cannot read properties of null (reading
// 'title')` — the loud half. A string, a number, a boolean and an array all RENDERED, 4579
// bytes, titled "Network Scan Report", with no company line — the quiet half, and the one a
// caller passing its company NAME where the brand OBJECT belongs would have shipped.
//
// A DEFAULTED PARAMETER (`brand = {}`) was rejected: it fires only on `undefined`, so it fixes
// the omitted-argument call and leaves an explicit `null` — the shape a caller that lost its
// brand actually produces — crashing exactly as before. The guard is a normalisation check at
// the single public entry point instead. `renderCover` and `renderBrandBlock` dereference
// `brand` freely — DERIVED, not counted by hand: 11 dereferences in this module, 7 in
// renderBrandBlock + 3 in renderCover + this one — and are reachable ONLY through here; that
// premise is not
...
export function renderExecutiveReport(model, brand, { renderedAt = new Date() } = {}) {
  if (brand === null || typeof brand !== 'object' || Array.isArray(brand)) {
    throw new TypeError(
      ...
      + "loadBrand()'s result, or `{}` for a deliberately unbranded report. It is "
      + `${describeBrand(brand)}. Refusing rather than rendering a client-facing report with `
      + 'the branding silently missing.');
  }
